  ...
  etap_mass(NULL)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"temp") == 0) {
      if (iarg+4+3 > narg) error->all(FLERR,"Illegal fix nvt/npt/nph command");
      tstat_flag = 1;
      ...
      // XLMD
      atom->XLMDFlag = force->inumeric(FLERR, arg[iarg+4]); // Songchen: 0 -> conventional; 1 -> XLMD
      atom->mLatent = force->numeric(FLERR, arg[iarg+5]); // Songchen: latent mass
      atom->tauLatent = force->numeric(FLERR, arg[iarg+6]);  // Songchen: latent coupling
      atom->tLatent = force->numeric(FLERR, arg[iarg+7]); // Songchen: latent temperature
...
void FixNH::final_integrate()
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) {
      if (atom->XLMDFlag) pLatent[i] += dth * fLatent[i];
    }
  }
...
double FixNH::kinetic_latent()
{
  ...
  for (int i = 0; i < nlocal; i++) {
    sum_p2 = sum_p2 + pLatent[i] * pLatent[i];
  }
  MPI_Allreduce(&sum_p2, &sum_p2_mpi, 1, MPI_DOUBLE, MPI_SUM, world);
  avg_v2 = sum_p2_mpi / atom->natoms / atom->mLatent / atom->mLatent;
  return avg_v2;
}

void FixNH::Berendersen(const double dt)
{
  ...
  double avg_v2 = kinetic_latent();
  double target_v2 = atom->tLatent / atom->mLatent;
  double scale = sqrt(1.0 + (dt/(atom->tauLatent))*(target_v2/avg_v2-1.0));
...
void FixNH::Langevin(const double dt) {
  int nlocal = atom->nlocal;
  double *pLatent;
  get_names("pLatent",pLatent); 
...
double FixNH::parallel_vector_acc(double *v)
{
  double my_acc, res;
  int nlocal = atom->nlocal;
